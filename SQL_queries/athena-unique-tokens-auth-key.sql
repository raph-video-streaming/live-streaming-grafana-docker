-- Total unique tokens from auth_key parameter over 5-minute periods
-- Shows distinct auth_key counts in 5-minute buckets

SELECT 
  date_trunc('minute', date_add('hour', -8, date_parse(date_time, '%d/%b/%Y:%H:%i:%s'))) as time_bucket,
  COUNT(DISTINCT regexp_extract(request_url, 'auth_key=([^&]+)', 1)) as unique_tokens,
...
WHERE 
  ...
  -- Ensure we have valid auth_key in URL
